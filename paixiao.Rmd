...
```{r paixiaoConstruction, echo = FALSE, fig.cap = 'PVC paixiao construction', fig.width=6, out.width = '50%',fig.align="center"}
knitr::include_graphics("img/paixiaoPVC_hotGlue.png")
```
...
```{r echo = FALSE}
keys<-c("C", "D", "E", "G", "A")
len<-c(154, 136, 120,98, 87)
keyDF<-data.frame(keys,len)
names(keyDF) <- c("Key", "Length (mm)")

knitr::kable(
  keyDF, booktabs = TRUE,
  caption = 'Musical key and estimated pipe length.'
)
```
